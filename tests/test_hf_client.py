"""Unit tests for HFGuard, the Hugging Face rate/budget guard (see
backend/ai_providers/base.py for why it exists: HF is the one AI tier in
this app with a real credit-limited budget, and the only one used as a
...
from backend.ai_providers.base import HFGuard
from backend.config import settings
from backend.curriculum import ALPHABET_TOPIC, LessonRequest, units_for_level
from backend.hf_client import _fallback_exercises, _fallback_review_exercises, _with_teaching_intros
...
def test_allowed_by_default():
    guard = HFGuard(daily_budget=1000)
    assert guard.allowed() is True


def test_budget_exhaustion_blocks_further_calls():
    guard = HFGuard(daily_budget=100)
...
def test_rate_limit_opens_circuit_immediately():
    guard = HFGuard(daily_budget=1000, cooldown_s=60.0)
    assert guard.allowed() is True
...
def test_circuit_recovers_after_cooldown_elapses():
    ...
    guard = HFGuard(daily_budget=1000, cooldown_s=-1.0)
    guard.record_rate_limited()
    assert guard.allowed() is True
